...
class home(LoginRequiredMixin, View):
    ...
    def get(self, request):
        # Fetch all service categories and feedbacks
        services = ServiceCatogarys.objects.all()
        feedbacks = Feedback.objects.select_related('User').all()

        # Convert QuerySet to a list
        all_services = list(ServiceCatogarys.objects.all())  # Get all services
        
        # Check if there are enough services to sample from
        num_services = min(len(all_services), 3)  # Choose 3 or less depending on the number of services available
        
        # If there are services available, select a random sample
        if all_services:
            selected_services = random.sample(all_services, num_services)
        else:
            selected_services = []  # If there are no services, return an empty list
        
        # Print for debugging purposes (can remove in production)
